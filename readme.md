...
1. **Token-Based Score Update**
   - Each request to update a score requires a unique, one-time token.
...
3. **Real-Time Updates**
   - On a successful score update, the server uses Socket.IO to broadcast the updated leaderboard to all clients.
...
### 1. **One-Time Token Authentication**
   - **Token Generation:**
     - Tokens are simulated as if generated by the client, rather than being created via the /score-token      endpoint.
...
### 4. **Rate Limiting and Abuse Detection**
   - Rate limits can be applied to the update score endpoint to prevent request spamming.
